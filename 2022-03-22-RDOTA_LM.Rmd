...
We pre-parse all the matches with ID: 3097027819 ...
...
# Extract radiant into: to simplicity, ignore the player order
...
game_data_df %>% mutate(delta = ifelse(Rscore > Dscore, 1, 0)) %>%
  filter(delta != Rwin)
```
...
## Relationship between game duration and Rwin

```{r}
game_data_df %>% 
  select(game_id, duration, Rwin) %>%
  mutate(Rwin_label = ifelse(Rwin == 1, "Radiant Win", "Dire Win")) %>%
  ggplot(aes(x = Rwin_label, y = duration)) + 
  geom_violin() + 
...
There are 114 heroes selected in each team, and we start from looking at the attendance frequency of the heroes from both team.  
...
top_gamers_df %>% dplyr::count(Rwin_label)

game_data_df %>% write.table("game_data_df.tsv", sep = "\t", quote = F, row.names = F)
...
Because we only looked at the top winning R gamers, not all the games would have 5 participants. Therefore we take a look at the heatmap first. 
...
We see the improvement of the model (R^2).

#############################################

Don't need to do the stepwise at current.

## TODO: stepwise linear regression

Instead of selecting gamers based on the wining game counts, we would also use stepwise linear regression to select the R heroes what are most relevant to the Rscore. 

Input data: game_data_df 

### Whether the heros information are independent [4]


```{r correlation coefficient alternative way}

pairwise_corr_df <- game_data_df %>% 
  select(R1:R114) %>% 
  as.matrix %>%
  cor %>%
  as.data.frame %>%
  rownames_to_column(var = 'hero1') %>%
  gather(hero2, correlation_coefficient, -hero1) %>%
  filter(hero1 != hero2) 

pairwise_corr_df %>% 
  ggplot(aes(x = correlation_coefficient)) + geom_histogram() + 
  theme_bw() + 
  scale_y_log10()
```


We need to remove highly correlated heros and only keep one. [5]

```{r}

library(vegan)
dist.mat <- vegdist(t(game_data_df %>% select(R1:R114)), method = "jaccard")
dist.mat <- as.matrix(dist.mat)

dist.df <- dist.mat %>% as.data.frame() %>% 
  rownames_to_column("hero2") %>% 
  gather(hero1, jaccard_distance, R1: R114) %>% 
  filter(hero1 != hero2) %>%
  filter(jaccard_distance < 0.9) %>% 
  spread(hero1, jaccard_distance, fill = 1)
rownames(dist.df) <- dist.df$hero2
dist.df %<>% select(-hero2)
...
```{r}
hero_attend <- game_data_df %>% select(game_id, R1:R114) %>% 
  gather(hero, attend, R1:R114) %>%
  group_by(hero) %>% summarise(attend = sum(attend))
pander(hero_attend)

my_data <- game_data_df %>% select(duration, R1:R113, Rscore)

#define intercept-only model
intercept_only <- lm(Rscore ~ 1, data=my_data)


#define model with all predictors
set.seed(101)
all <- lm(Rscore ~ ., data=my_data)
summary(all)
...
#view results of forward stepwise linear regression
forward$anova
